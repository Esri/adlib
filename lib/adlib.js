...
function _loadTemplate(path) {
    // TODO: support either local or URI references
    return require(path)
}
...
function _swap(parameter, settings) {
    ...
    // If necessary, call transform methods
...
export default function adlib(template, settings) {
    ...
    // First load external references
    if(_.has(template, '...')) {
        template = _loadTemplate(template['...'])
    }

    return _.deepMapValues(template, function(templateValue, templatePath){
