...
function _swap(parameter, settings, transforms) {
  ...
  if (transformCheck.length > 1) {
    ...
    if (transformCheck[2]){
      transformParam = transformCheck[2];
    }
    // get the value from the param
    value = getWithDefault(settings, key);
    if (transforms && transforms[fn] && typeof transforms[fn] === 'function') {
      // if we have a value, or it's the optional transform...
      if (value || fn === 'optional') {
        // transform it...
        value = transforms[fn](key, value, settings, transformParam);
      } else {
        if (!value) {
      ...
      // no transform, but we have a value...
      if (value) {
